...
class Apriltag {
   ...
   */
    constructor(onDetectorReadyCallback) {
        ...
        // detector options
        this._opt = {
          // Decimate input image by this factor (2.0 = 4x fewer pixels, faster but slightly less accurate)
          quad_decimate: 2.0,
          ...
          return_solutions: 1,
          // Minimum decision margin threshold (filter out low-confidence detections)
          min_margin: 20.0
        }
        ...
        AprilTagWasm().then(function (Module) {
            ...
            console.error("Failed to load AprilTag WASM:", err);
        });
    }
    ...
    detect(grayscaleImg, imgWidth, imgHeight) {
        ...
        // Filter by decision_margin threshold
        if (this._opt.min_margin > 0) {
            detections = detections.filter(det =>
                det.decision_margin && det.decision_margin >= this._opt.min_margin
            );
        }
    ...
    /**
     * **public** detect from RGBA data (grayscale conversion done in worker)
     * @param {Uint8Array|Uint8ClampedArray} rgbaData RGBA pixel data
     * @param {Number} imgWidth image width
     * @param {Number} imgHeight image height
     * @return {detection} detection object
     */
    detectRGBA(rgbaData, imgWidth, imgHeight) {
        if (!this._Module) {
            console.error("WASM module not initialized yet");
            return [];
        }

        // Fast grayscale conversion in worker (avoids main thread blocking)
        const pixelCount = imgWidth * imgHeight;
        const grayscale = new Uint8Array(pixelCount);

        // Optimized loop: process 4 pixels at a time when possible
        let grayIdx = 0;
        let rgbaIdx = 0;
        const len = pixelCount << 2; // pixelCount * 4

        // Unrolled loop for better performance
        while (rgbaIdx < len) {
            grayscale[grayIdx++] = (rgbaData[rgbaIdx] + rgbaData[rgbaIdx + 1] + rgbaData[rgbaIdx + 2]) / 3 | 0;
            rgbaIdx += 4;
        }

        // Now detect using the grayscale buffer
        let imgBuffer = this._set_img_buffer(imgWidth, imgHeight, imgWidth);
        this._Module.HEAPU8.set(grayscale, imgBuffer);
